/* ============================================================================
   MATES — skript stránek
   Načítá obsah z obsah.js a vykresluje ho do stránek. Tento soubor NEMUSÍTE
   upravovat — veškerý obsah se mění v obsah.js.
   ============================================================================ */
...
(function () {
  ...
  /* Text s odstavci: v obsah.js se odstavce oddělují prázdným řádkem (\n\n) */
  function paragraphs(text) {
    return String(text || "").split(/\n\s*\n/);
  }

  /* ---------- Kontakt: vyplní elementy s data-z ---------- */
  ...
  /* Krátký úryvek: první odstavec oříznutý na ~150 znaků */
  ...
  /* Tělo příspěvku: odstavce + případné tlačítko s odkazem */
  function postBody(a) {
  ...
  /* Karta s celým příspěvkem (úvodní stránka i stránka Všechny příspěvky) */
  function fullCard(a, id) {
    var card = el("article", "worksheet worksheet--full");
    if (id) card.id = id;
    card.appendChild(el("span", "tape")).setAttribute("aria-hidden", "true");
  ...
  /* Kompaktní karta: úryvek + tlačítko Rozbalit/Zabalit */
  ...
  /* Příspěvky: úvodní stránka (3 nejnovější) nebo stránka se všemi */
  ...
  /* Odebere diakritiku a převede na malá písmena („serie“ najde i „série“) */
  function norm(s) {
    return String(s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  }

  /* Vyhledávání — lupa v menu, vysouvací panel pod hlavičkou */
  function initSearch() {
    ...
    function doSearch(input, results) {
      ...
      (DATA.aktuality || []).forEach(function (a, i) {
        if (hit(norm(a.nadpis + " " + a.text), q)) {
          hits.push({ type: "Příspěvek" + (a.datum ? " · " + a.datum : ""), label: a.nadpis, href: "vsechny-prispevky.html#post-" + i });
        }
      });
      ...
      (DATA.archiv || []).forEach(function (rok) {
        (rok.serie || []).forEach(function (s) {
          if (hit(norm(s.nazev + " " + rok.rocnik), q)) {
            hits.push({ type: "Archiv · " + rok.rocnik, label: s.nazev, href: "archiv.html" });
          }
        });
      });
      ...
      (DATA.oNas || []).forEach(function (o) {
        if (hit(norm(o.nadpis + " " + o.text), q)) {
          hits.push({ type: "O soutěži", label: o.nadpis, href: "o-soutezi.html" });
        }
      });

      /* Stránka se všemi příspěvky — najdeš ji i vyhledáváním */
      if (q.indexOf("prispev") !== -1 || q.indexOf("vsechny") !== -1 || q.indexOf("posts") !== -1) {
        hits.unshift({ type: "Stránka", label: "Všechny příspěvky", href: "vsechny-prispevky.html" });
      }
  ...
  function main() {
    if (!DATA) {
      showError("Nepodařilo se načíst obsah. Zkontrolujte, že soubor obsah.js je ve stejné složce jako stránka — a jestli v něm nechybí čárka, uvozovka nebo závorka (návod v NAVOD.md).");
      initMenu();
      return;
    }
    try {
      ...
      showError("Něco se nepovedlo při vykreslování stránky (" + err.message + "). Podívejte se do NAVOD.md.");
      if (window.console) console.error(err);
    }
  }

  /* ---------- Téma (světlý/tmavý) a stín hlavičky ---------- */

  /* Logo v menu je tmavé — v tmavém režimu ho vyměníme za bílou verzi */
  ...
  /* Zpřístupní vykreslení příspěvků editovacímu režimu (editor.js) */
  window.MATESUI = {
